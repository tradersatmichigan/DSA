...
class skip_list {
  ...
  skip_list* _insert(int value, int level = -1) {
    if (level < 0)
      level = next.size() - 1;
    skip_list* it;
    if (next[level] && next[level]->value < value)
      it = next[level]->_insert(value, level);
    else if (level == 0) {
      int promotions = 0;
      while (flipCoin())
        ++promotions;
  ...
  skip_list* _search(int i, int level = -1) {
    ...
    if (next[level] && next[level]->value <= i)
      return next[level]->_search(i, level);
    else if (level == 0) {
      if (i == value)
        return this;
      else
 ...
 public:
  ...
  skip_list* search(int i) { return _search(i, -1); }
  ...
  void printLevel(int level) {
    if (level < next.size())
      ...
      std::stringstream s;
      s << value;
      for (int i = 0; i < s.str().size() + 1; ++i)
        printf(" ");
    }
    if (next[0])
      next[0]->printLevel(level);
  }
  void print() {
